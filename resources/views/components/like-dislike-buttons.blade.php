<div class="flex">
    ...
    @if(auth()->check() && $tweet->isDisliked())
        <form method="POST"
            action="{{ route('dislikes.destroy', $tweet) }}"
        >
        @method('DELETE')        
    @else
        <form method="POST"
            action="{{ route('dislikes.store', $tweet) }}"
        >
    @endif
    @csrf

    @auth
        <div class="flex items-center mr-4 {{ $tweet->isDisliked() ? 'text-red-500' : 'text-gray-500' }}"> 
    @else
        <div class="flex items-center mr-4 text-gray-500">
    @endauth
